...
var fetch = exports.fetch = function fetch(url) {
  ...
  var finalOptions = Object.assign({}, options, getDefaultFetchOpts(options));

  // https://m.alphasights.com/killing-cors-preflight-requests-on-a-react-spa-1f9b04aa5730#4bdf
  // eslint-disable-next-line prefer-template
  var finalUrl = url + (url.includes('?') ? '&' : '?') + ('bearer_token=' + user.access_token);
...
function getDefaultFetchOpts(options) {
  ...
  return {
    headers: Object.assign({ Accept: 'application/json; charset=utf-8' }, !isCorsSimpleMethod ? {
